...
本项目基于昨天的<a href='https://github.com/JetXi/JiangXi-University-Antiepidemic-Clock-In'>江西高校支付宝校园防疫自动签到</a>新增了批量签到功能（瞎改了一下），将要批量签到的同学信息填入excel表中，注意不要多出空格或其他字符，保存后运行程序即可完成批量签到。选用csv主要是为了方便Linux环境下操作，没有安装excel的同学也可以直接使用记事本编辑，以逗号分隔各数据项即可。



有开发能力的朋友可以考虑绑定一个微信机器人以实时推送签到情况，因为我个人使用的是一个小众的机器人，不便推荐，便删除了发布版本中的相关代码。将代码挂在Windows计划任务或Linux crontab下都可以实现定时自动签到。顺便推荐下腾讯云云函数，免费好用，把脚本上云之后不占用本地资源，也不需要频繁检查运行状态。
...
| ---------------- | -------- | ------ | ---- | ----- | ------------------------------------------------------------ | ------------------------------------------------ | ------------------------------------------------ |
| 可在文档末尾查询 | 完整学号 | 江西省 | xx市 | xx区  | 详细的定位信息，可以参考校园防疫小程序中的定位结果，同时要保证具体地址和经纬度之间大致可以互相转换 | 可由在线坐标拾取系统查询，建议保留五位以上的精度 | 可由在线坐标拾取系统查询，建议保留五位以上的精度 |
